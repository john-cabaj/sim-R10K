 ...
 *
 * $Id: misc.c,v 1.6 1998/08/27 15:41:27 taustin Exp taustin $
 *
 * $Log: misc.c,v $
...
mystricmp(const char *s1, 
	  const char *s2)/* strings to compare, case insensitive */
{
  unsigned char u1, u2;
...
unsigned int 
floor_log_base2(unsigned int n)
{
  unsigned int power = 0;
  while (n >>= 1) power++;
  return power;
...
/* portable fprintf with quadword support, returns end pointer */
void
mygzprintf(gzFile stream, char *format, ...)
{
  /* temp buffer */
  char buf[512];
...
{
  byte_t *b = (byte_t *) calloc (nmemb, size);
