 ...
 *
 * $Id: loader.c,v 1.1 1998/08/27 16:54:16 taustin Exp taustin $
 *
 * $Log: loader.c,v $
...
void
ld_stats(FILE *stream)	/* stats data base */
{
  print_addr(stream, "ld_text_base", ld_text_base, "program text (code) segment base");
  print_int(stream, "ld_text_size", ld_text_size, "program text (code) size in bytes");
  print_addr(stream, "ld_data_base", ld_data_base, "program initialized data segment base");
  print_int(stream, "ld_data_size", ld_data_size, "program init'ed `.data' and uninit'ed `.bss' size in bytes");
  print_addr(stream, "ld_stack_base", ld_stack_base, "program stack segment base (highest address in stack)");
#if 0 /* FIXME: broken... */
  print_addr(stream, "ld_stack_min", ld_stack_min, "program stack segment lowest address");
#endif
  print_int(stream, "ld_stack_size", ld_stack_size, "program initial stack size");
  print_addr(stream, "ld_prog_entry", ld_prog_entry, "program entry point (initial PC)");
  print_addr(stream, "ld_environ_base", ld_environ_base, "program environment base address address");
  print_int(stream, "ld_target_big_endian", ld_target_big_endian, "target executable endian-ness, non-zero if big endian");
}
	     ...
	     int zero_bss_segs)		/* zero uninit data segment? */
{
  ...
  if (eio_valid(fname))
      ...
      if (sim_chkpt_fname != NULL)
	{
      ...
      /* computed state... */
      ld_environ_base = regs->regs[MD_REG_SP].q;
      ld_prog_entry = regs->PC;
...

  {
	...
	if ((bfd_get_section_flags(abfd, sect) & SEC_ALLOC)
	    ...
	    && bfd_section_size(abfd, sect))
	  {
	    ...
	    mem_bcopy(mem_fn, mc_WRITE, bfd_section_vma(abfd, sect),
...

  {
    ...
    regs->regs[MD_REG_GP].q = ahdr.gp_value;
	...
	switch (shdr.s_flags)
	  ...
	  case ECOFF_STYP_TEXT:
	    ...
	    mem_bcopy(mem_access, mem, mc_WRITE, shdr.s_vaddr, p, shdr.s_size);
	  ...
	  case ECOFF_STYP_FINI:
		...
		mem_bcopy(mem_access, mem,
			  mc_WRITE, shdr.s_vaddr, p, shdr.s_size);
  ...
  /* write [argc] to stack */
  temp = SWAP_WORD(argc);
  mem_access(mem, mc_WRITE, sp, &temp, sizeof(quad_t));
  regs->regs[MD_REG_A0].q = temp;
  sp += sizeof(quad_t);

  /* skip past argv array and NULL */
  argv_addr = sp;
  regs->regs[MD_REG_A1].q = argv_addr;
  ...
  for (i=0; i<argc; i++)
    {
      ...
      temp = SWAP_WORD(sp);
      mem_access(mem, mc_WRITE, argv_addr + i*sizeof(md_addr_t),
		 &temp, sizeof(md_addr_t));
      /* and the data */
      mem_strcpy(mem_access, mem, mc_WRITE, sp, argv[i]);
      sp += strlen(argv[i])+1;
    }
  ...
  for (i = 0; envp[i]; i++)
    {
      ...
      temp = SWAP_WORD(sp);
      mem_access(mem, mc_WRITE, envp_addr + i*sizeof(md_addr_t),
		 &temp, sizeof(md_addr_t));
      /* and the data */
      mem_strcpy(mem_access, mem, mc_WRITE, sp, envp[i]);
      sp += strlen(envp[i]) + 1;
    }
  ...
  ld_stack_min = regs->regs[MD_REG_SP].q;

  regs->regs[MD_REG_SP].q = ld_environ_base;
