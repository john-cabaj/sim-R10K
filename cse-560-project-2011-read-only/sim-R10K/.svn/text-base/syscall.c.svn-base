 ...
 *
 * $Id: syscall.c,v 1.3 1998/09/03 22:20:59 taustin Exp taustin $
 *
 * $Log: syscall.c,v $
 * Revision 1.3  1998/09/03 22:20:59  taustin
 * iov_len field in osf_iov fixed (was quad_t changed to word_t)
...
{
  quad_t syscode = regs->regs[MD_REG_V0].q;

  /* 
   * -- Fix for syscall() which uses CALL_PAL CALLSYS for making
   * -- system calls
   */
  if( syscode == OSF_SYS_syscall )
  {
     syscode = regs->regs[MD_REG_A0].q;
  }

  /* first, check if an EIO trace is being consumed... */
  if (traceable && sim_eio_fd != NULL)
    {
      md_addr_t sc_addr = regs->regs[MD_REG_A0].q;
      ...
      if (syscode == OSF_SYS_sigreturn) 
	{
	  ...
	  mem_bcopy(mem_fn, mem, mc_READ, sc_addr,
		    &sc, sizeof(struct osf_sigcontext));
	  regs->NPC = sc.sc_pc;
	  for (i=0; i < 32; ++i) regs->regs[i].q = sc.sc_regs[i]; 
	  for (i=0; i < 32; ++i) regs->regs[i+MD_NUM_IREGS].q = sc.sc_fpregs[i]; 
	  regs->regs[DFPCR].q = sc.sc_fpcr;
	}
  ...
  switch (syscode)
    ...
    case OSF_SYS_exit:
      ...
      longjmp(sim_exit_buf,
	      /* exitcode + fudge */(regs->regs[MD_REG_A0].q & 0xff) + 1);
      break;
	...
	if (!(buf =
	      (char *)calloc(/*nbytes*/regs->regs[MD_REG_A2].q, sizeof(char))))
	  fatal("out of memory in SYS_read");

	/* read data from file */
	do {
	  /*nread*/regs->regs[MD_REG_V0].q =
	    read(/*fd*/regs->regs[MD_REG_A0].q, buf,
	         /*nbytes*/regs->regs[MD_REG_A2].q);
	} while (/*nread*/regs->regs[MD_REG_V0].q == -1
	         && errno == EAGAIN);

	/* check for error condition */
	if (regs->regs[MD_REG_V0].q != (quad_t)-1)
	  regs->regs[MD_REG_A3].q = 0;
	else /* got an error, return details */
	  {
	    regs->regs[MD_REG_A3].q = -1;
	    regs->regs[MD_REG_V0].q = errno;
	  }

	/* copy results back into host memory */
	mem_bcopy(mem_fn, mem, mc_WRITE,
		  /*buf*/regs->regs[MD_REG_A1].q, buf, /*nread*/regs->regs[MD_REG_A2].q);
	...
	mem_bcopy(mem_fn, mem, mc_READ, /*buf*/regs->regs[MD_REG_A1].q, buf,
		  /*nbytes*/regs->regs[MD_REG_A2].q);
	...
	if (sim_progfd && MD_OUTPUT_SYSCALL(regs))
	  {
	    ...
	    /*nwritten*/regs->regs[MD_REG_V0].q =
	      fwrite(buf, 1, /*nbytes*/regs->regs[MD_REG_A2].q, sim_progfd);
	  }
	else
	  {
	    ...
	    do {
	      /*nwritten*/regs->regs[MD_REG_V0].q =
	        write(/*fd*/regs->regs[MD_REG_A0].q,
		      buf, /*nbytes*/regs->regs[MD_REG_A2].q);
	    } while (/*nwritten*/regs->regs[MD_REG_V0].q == -1
		     && errno == EAGAIN);
	  }

	/* check for an error condition */
	if (regs->regs[MD_REG_V0].q == regs->regs[MD_REG_A2].q)
	  regs->regs[MD_REG_A3].q = 0;
	else /* got an error, return details */
	  {
	    regs->regs[MD_REG_A3].q = -1;
	...
	char buf[MAXBUFSIZE];
	unsigned int i;
	int osf_flags = regs->regs[MD_REG_A1].q, local_flags = 0;
	...
	mem_strcpy(mem_fn, mem, mc_READ, /*fname*/regs->regs[MD_REG_A0].q, buf);

	/* open the file */
#ifdef __CYGWIN32__
	/*fd*/regs->regs[MD_REG_V0].q =
	  open(buf, local_flags|O_BINARY, /*mode*/regs->regs[MD_REG_A2].q);
#else /* !__CYGWIN32__ */
	/*fd*/regs->regs[MD_REG_V0].q =
	  open(buf, local_flags, /*mode*/regs->regs[MD_REG_A2].q);
#endif /* __CYGWIN32__ */
    ...
    case OSF_SYS_close:
      ...
      if (/*fd*/regs->regs[MD_REG_A0].q == 0
	  || /*fd*/regs->regs[MD_REG_A0].q == 1
	  || /*fd*/regs->regs[MD_REG_A0].q == 2)
	{
	  regs->regs[MD_REG_A3].q = 0;
	  break;
	}
      ...
      /* close the file */
      regs->regs[MD_REG_V0].q = close(/*fd*/regs->regs[MD_REG_A0].q);
	...
	/* copy filename to host memory */
	mem_strcpy(mem_fn, mc_READ, /*fname*/regs->regs[MD_REG_A0].q, buf);

	/* create the file */
	/*fd*/regs->regs[MD_REG_V0].q =
	  creat(buf, /*mode*/regs->regs[MD_REG_A1].q);
	...
	/* delete the file */
	/*result*/regs->regs[MD_REG_V0].q = unlink(buf);
	...
	/*result*/regs->regs[MD_REG_V0].q = chdir(buf);
	...
	/* chmod the file */
	/*result*/regs->regs[MD_REG_V0].q =
	  chmod(buf, /*mode*/regs->regs[MD_REG_A1].q);
	...
	/* chown the file */
	/*result*/regs->regs[MD_REG_V0].q =
	  chown(buf, /*owner*/regs->regs[MD_REG_A1].q,
		/*group*/regs->regs[MD_REG_A2].q);
	...
	ld_brk_point = addr;
	regs->regs[MD_REG_V0].q = ld_brk_point;
	regs->regs[MD_REG_A3].q = 0;
	...
	if (/* addr >= ld_brk_point && */ addr < regs->regs[MD_REG_SP].q)
	  {
	    regs->regs[MD_REG_A3].q = 0;
	    ld_brk_point = addr;
	  }
...
#if 0
        addr = ROUND_UP(/*base*/regs->regs[MD_REG_A0].q, MD_PAGE_SIZE);
#endif
        addr = /*base*/regs->regs[MD_REG_A0].q;
    ...
    case OSF_SYS_lseek:
      /* seek into file */
      regs->regs[MD_REG_V0].q =
	lseek(/*fd*/regs->regs[MD_REG_A0].q,
	      /*off*/regs->regs[MD_REG_A1].q, /*dir*/regs->regs[MD_REG_A2].q);
    ...
    case OSF_SYS_getpid:
      ...
      /*result*/regs->regs[MD_REG_V0].q = getpid();
...
#else /* !_MSC_VER */
      /* get current user id */
      /*first result*/regs->regs[MD_REG_V0].q = getuid();
	...
	/*result*/regs->regs[MD_REG_V0].q =
	  access(buf, /*mode*/regs->regs[MD_REG_A1].q);
	...
	if (syscode == OSF_SYS_stat)
	  /*result*/regs->regs[MD_REG_V0].q = stat(buf, &sbuf);
	else /* syscode == OSF_SYS_lstat */
	  {
            ...
            break;
#else /* !_MSC_VER */
	    /*result*/regs->regs[MD_REG_V0].q = lstat(buf, &sbuf);
#endif /* _MSC_VER */
	  }
	...
	mem_bcopy(mem_fn, mem, mc_WRITE, /*sbuf*/regs->regs[MD_REG_A1].q,
    ...
    case OSF_SYS_dup:
      ...
      /*fd*/regs->regs[MD_REG_V0].q = dup(/*fd*/regs->regs[MD_REG_A0].q);
	...
	mem_bcopy(mem_fn, mem, mc_READ, /*fd's*/regs->regs[MD_REG_A0].q,
		  fd, sizeof(fd));

	/* create a pipe */
	/*result*/regs->regs[7].q = pipe(fd);

	/* copy descriptor results to result registers */
	/*pipe1*/regs->regs[MD_REG_V0].q = fd[0];
	/*pipe 2*/regs->regs[3].q = fd[1];

	/* check for an error condition */
	if (regs->regs[7].q == (quad_t)-1)
	  {
	    regs->regs[MD_REG_V0].q = errno;
	    regs->regs[7].q = 1;
	  }
    ...
    case OSF_SYS_getgid:
      ...
      /*first result*/regs->regs[MD_REG_V0].q = getgid();

#if 0
      /* get current effective group id */
      /*second result*/regs->regs[3].q = getegid();
#endif
    ...
    case OSF_SYS_ioctl:
      switch (/* req */regs->regs[MD_REG_A1].q)
	{
#ifdef TIOCGETP
	case OSF_TIOCGETP: /* <Out,TIOCGETP,6> */
	  {
	    ...
	    /* result */regs->regs[MD_REG_V0].q =
#ifdef linux
		bsd_ioctl(/* fd */(int)regs->regs[MD_REG_A0].q,
#else 		  
		ioctl(/* fd */(int)regs->regs[MD_REG_A0].q,
#endif
		    /* req */TIOCGETP,
		    &lbuf);
...
/* #endif */
	...
	if (!local_req)
	  {
	    ...
	    warn("syscall: ioctl: ioctl code not supported d=%d, req=%d",
		 regs->regs[MD_REG_A0].q, regs->regs[MD_REG_A1].q);
	    regs->regs[MD_REG_V0].q = 0;
	    regs->regs[7].q = 0;
	  }
	else
	  {
	    ...
	    if (/*argp*/regs->regs[MD_REG_A2].q != 0)
	      mem_bcopy(mem_fn, mem, mc_READ, /*argp*/regs->regs[MD_REG_A2].q,
			buf, NUM_IOCTL_BYTES);

	    /* perform the ioctl() call */
	    /*result*/regs->regs[MD_REG_V0].q =
	      ioctl(/*fd*/regs->regs[MD_REG_A0].q, local_req, buf);

	    /* if arg ptr exists, copy NUM_IOCTL_BYTES bytes from host mem */
	    if (/*argp*/regs->regs[MD_REG_A2].q != 0)
	      mem_bcopy(mem_fn, mem, mc_WRITE, regs->regs[MD_REG_A2].q,
			buf, NUM_IOCTL_BYTES);
	...
	/* fstat() the file */
	/*result*/regs->regs[MD_REG_V0].q =
	  fstat(/*fd*/regs->regs[MD_REG_A0].q, &sbuf);
    ...
    case OSF_SYS_getpagesize:
      /* get target pagesize */
      regs->regs[MD_REG_V0].q = /* was: getpagesize() */MD_PAGE_SIZE;
    ...
    case OSF_SYS_table:
      {
	quad_t table_id, table_index, buf_addr, num_elem, size_elem;
	struct osf_tbl_sysinfo sysinfo;
	
	table_id = regs->regs[MD_REG_A1].q;
	table_index = regs->regs[MD_REG_A2].q;
	buf_addr = regs->regs[MD_REG_A3].q;
	num_elem = regs->regs[MD_REG_A4].q;
	size_elem = regs->regs[MD_REG_A5].q;
	...
	switch( table_id )
	{
	  case OSF_TBL_SYSINFO:
	    if( table_index != 0 )
	    {
	      panic("table: table id TBL_SYSINFO requires table index of zero, instead got %08d\n", table_index );
	    }
	    else if( num_elem != 1 )
	    {
	      panic("table: table id TBL_SYSINFO requires num of elements = 1, instead got %08d\n", num_elem );
	    }
	    else
	    {
		...
		regs->regs[MD_REG_A3].q = -1;
		regs->regs[MD_REG_V0].q = errno; /* -- Not kosher to pass off errno of getrusage()
						  * -- as errno of table(), but what the heck ...
						  */
		break;
	      }
	      ...
	      mem_bcopy(mem_fn, mem, mc_WRITE, buf_addr, &sysinfo, sizeof(struct osf_tbl_sysinfo));
	  ...
	  default:
	    warn("table: unsupported table id %d requested, ignoring!\n", table_id);
	    regs->regs[MD_REG_A3].q = 0;
	}
    ...
    case OSF_SYS_getdtablesize:
      ...
      regs->regs[MD_REG_V0].q = getdtablesize();
	...
	if (getrlimit(RLIMIT_NOFILE, &rl) != (quad_t)-1)
	  {
	    regs->regs[MD_REG_V0].q = rl.rlim_cur;
	    regs->regs[MD_REG_A3].q = 0;
	  }
	else /* got an error, return details */
	  {
    ...
    case OSF_SYS_dup2:
      ...
      regs->regs[MD_REG_V0].q =
	dup2(/*fd1*/regs->regs[MD_REG_A0].q, /*fd2*/regs->regs[MD_REG_A1].q);
    ...
    case OSF_SYS_fcntl:
      ...
      regs->regs[MD_REG_V0].q =
	fcntl(/*fd*/regs->regs[MD_REG_A0].q,
	      /*cmd*/regs->regs[MD_REG_A1].q, /*arg*/regs->regs[MD_REG_A2].q);
	...
	if (/*timeval*/regs->regs[MD_REG_A0].q != 0)
	  {
	    /* copy timeval into host memory */
	    mem_bcopy(mem_fn, mem, mc_READ, /*timeval*/regs->regs[MD_REG_A0].q,
	...
	if (/*timezone*/regs->regs[MD_REG_A1].q != 0)
	  {
	    /* copy timezone into host memory */
	    mem_bcopy(mem_fn, mem, mc_READ, /*timezone*/regs->regs[MD_REG_A1].q,
	...
	/* get time of day */
	/*result*/regs->regs[MD_REG_V0].q = gettimeofday(tvp, tzp);
	...
	if (/*timeval*/regs->regs[MD_REG_A0].q != 0)
	  {
	    ...
	    mem_bcopy(mem_fn, mem, mc_WRITE, /*timeval*/regs->regs[MD_REG_A0].q,
		      &osf_tv, sizeof(struct osf_timeval));
	  }
	...
	if (/*timezone*/regs->regs[MD_REG_A1].q != 0)
	  {
	    ...
	    mem_bcopy(mem_fn, mem, mc_WRITE, /*timezone*/regs->regs[MD_REG_A1].q,
		      &osf_tz, sizeof(struct osf_timezone));
	  }
      }
#endif /* !_MSC_VER */
	...
	mem_bcopy(mem_fn, mem, mc_WRITE, /*rusage*/regs->regs[MD_REG_A1].q,
	...
	/*result*/regs->regs[MD_REG_V0].q =
	  getrusage(/*who*/regs->regs[MD_REG_A0].q, &local_rusage);
...
#error No utimes() implementation!
	...
	else
	  {
	    ...
	    mem_bcopy(mem_fn, mem, mc_READ, /*timeout*/regs->regs[MD_REG_A1].q,
	    ...
	    /* no utimes() in hpux, use utime() instead */
	    {
	      ...
	      /* result */regs->regs[MD_REG_V0].q = utime(buf, &ubuf);
	    }
#elif defined(_MSC_VER)
            /* no utimes() in hpux, use utime() instead */
            {
	    ...
	    /* result */regs->regs[MD_REG_V0].q = utimes(buf, tval);
#elif defined(__CYGWIN32__)
	    warn("syscall: called utimes\n");
#else
#error No utimes() implementation!
	...
	mem_bcopy(mem_fn, mem, mc_READ, /*rlimit*/regs->regs[MD_REG_A1].q,
	...
	if (syscode == OSF_SYS_getrlimit)
	  /*result*/regs->regs[MD_REG_V0].q =
	    getrlimit(regs->regs[MD_REG_A0].q, &rl);
	else /* syscode == OSF_SYS_setrlimit */
	  /*result*/regs->regs[MD_REG_V0].q =
	    setrlimit(regs->regs[MD_REG_A0].q, &rl);
	...
	mem_bcopy(mem_fn, mem, mc_WRITE, /*rlimit*/regs->regs[MD_REG_A1].q,
		  &osf_rl, sizeof(struct osf_rlimit));
      }
#endif
      break;
	...
	/* from klauser@cs.colorado.edu: there are a couple bugs in the
	   ...
	   handle it. (e.g. you can't really ever block sigkill etc.) */
        ...
        regs->regs[MD_REG_V0].q = sigmask;
        regs->regs[MD_REG_A3].q = 0;

        switch (regs->regs[MD_REG_A0].q)
	  {
          case OSF_SIG_BLOCK:
            sigmask |= regs->regs[MD_REG_A1].q;
            break;
          ...
          case OSF_SIG_SETMASK:
            ...
            break;
          default:
            regs->regs[MD_REG_V0].q = EINVAL;
            regs->regs[MD_REG_A3].q = 1;
	  }

#if 0 /* FIXME: obsolete... */
	if (regs->regs[MD_REG_A2].q > /* FIXME: why? */0x10000000)
	  mem_bcopy(mem_fn, mem, mc_WRITE, regs->regs[MD_REG_A2].q,
		    &sigmask, sizeof(sigmask));

	if (regs->regs[MD_REG_A1].q != 0)
	  {
	    switch (regs->regs[MD_REG_A0].q)
	      {
	      case OSF_SIG_BLOCK:
		...
		break;
	      case OSF_SIG_UNBLOCK:
		sigmask &= regs->regs[MD_REG_A1].q; /* I think */
	      break;
	      case OSF_SIG_SETMASK:
		sigmask = regs->regs[MD_REG_A1].q; /* I think */
		break;
	...
	signum = regs->regs[MD_REG_A0].q;
	if (regs->regs[MD_REG_A1].q != 0)
	  sigaction_array[signum] = regs->regs[MD_REG_A1].q;

	if (regs->regs[MD_REG_A2].q)
	  regs->regs[MD_REG_A2].q = sigaction_array[signum];
	...
	mem_bcopy(mem_fn, mem, mc_READ, /* sc */regs->regs[MD_REG_A0].q,
		  &sc, sizeof(struct osf_sigcontext));
	...
	for (i=0; i < 32; ++i)
	  regs->regs[i].q = sc.sc_regs[i];
	for (i=0; i < 32; ++i)
	  regs->regs[i+MD_NUM_IREGS].q = sc.sc_fpregs[i];
	regs->regs[DFPCR].q = sc.sc_fpcr;
      }
      break;
    ...
    case OSF_SYS_uswitch:
      ...
      regs->regs[MD_REG_V0].q = regs->regs[MD_REG_A1].q; 
      break;
	...
	int i, cnt, osf_cnt;
	struct dirent *p;
	sword_t fd = regs->regs[MD_REG_A0].q;
	md_addr_t osf_buf = regs->regs[MD_REG_A1].q;
	char *buf;
	sword_t osf_nbytes = regs->regs[MD_REG_A2].q;
	md_addr_t osf_pbase = regs->regs[MD_REG_A3].q;
	squad_t osf_base;
	long base = 0;
	...
	if (regs->regs[MD_REG_V0].q != (quad_t)-1)
	  {
		...
		for (i=0, cnt=0, osf_cnt=0, p=(struct dirent *)buf;
		     ...
		     i++, cnt += p->d_reclen, p=(struct dirent *)(buf+cnt))
		  {
		    ...
		    mem_bcopy(mem_fn, mem, mc_WRITE,
			      osf_buf + osf_cnt,
			      &osf_dirent, osf_dirent.d_reclen);
		    osf_cnt += osf_dirent.d_reclen;
		  }

		if (osf_pbase != 0)
		  {
		    osf_base = (squad_t)base;
		    mem_bcopy(mem_fn, mem, mc_WRITE, osf_pbase,
			      &osf_base, sizeof(osf_base));
		  }

		/* update V0 to indicate translated read length */
		regs->regs[MD_REG_V0].q = osf_cnt;
	      }
	  }
	else /* got an error, return details */
	  {
	...
	/* truncate the file */
	/*result*/regs->regs[MD_REG_V0].q =
	  truncate(buf, /* length */(size_t)regs->regs[MD_REG_A1].q);
    ...
    case OSF_SYS_ftruncate:
      /* truncate the file */
      /*result*/regs->regs[MD_REG_V0].q =
	ftruncate(/* fd */(int)regs->regs[MD_REG_A0].q,
		 /* length */(size_t)regs->regs[MD_REG_A1].q);
	...
	/* statfs() the fs */
	/*result*/regs->regs[MD_REG_V0].q = statfs(buf, &sbuf);
	...
	/* translate from host stat structure to target format */
#if defined(__svr4__) || defined(__osf__)
	osf_sbuf.f_type = 0x6969 /* NFS, whatever... */;
#else /* !__svr4__ */
    ...
    case OSF_SYS_setregid:
      ...
      /*result*/regs->regs[MD_REG_V0].q =
	setregid(/* rgid */(gid_t)regs->regs[MD_REG_A0].q,
		 /* egid */(gid_t)regs->regs[MD_REG_A1].q);
    ...
    case OSF_SYS_setreuid:
      ...
      /*result*/regs->regs[MD_REG_V0].q =
	setreuid(/* ruid */(uid_t)regs->regs[MD_REG_A0].q,
		 /* euid */(uid_t)regs->regs[MD_REG_A1].q);
    ...
    case OSF_SYS_socket:
      ...
      /* result */regs->regs[MD_REG_V0].q =
	socket(/* domain */xlate_arg((int)regs->regs[MD_REG_A0].q,
				     family_map, N_ELT(family_map),
				     "socket(family)"),
	       /* type */xlate_arg((int)regs->regs[MD_REG_A1].q,
				   socktype_map, N_ELT(socktype_map),
				   "socket(type)"),
	       /* protocol */xlate_arg((int)regs->regs[MD_REG_A2].q,
				       family_map, N_ELT(family_map),
				       "socket(proto)"));
	...
	if (regs->regs[MD_REG_A2].q > sizeof(struct osf_sockaddr))
	  {
	    fatal("sockaddr size overflow: addrlen = %d",
		  regs->regs[MD_REG_A2].q);
	  }
	/* copy sockaddr structure to host memory */
	mem_bcopy(mem_fn, mem, mc_READ, /* serv_addr */regs->regs[MD_REG_A1].q,
		  &osf_sa, /* addrlen */(int)regs->regs[MD_REG_A2].q);
#if 0
	int i;
	sa.sa_family = osf_sa.sa_family;
	for (i=0; i < regs->regs[MD_REG_A2].q; i++)
	  sa.sa_data[i] = osf_sa.sa_data[i];
#endif
	/* result */regs->regs[MD_REG_V0].q =
	  connect(/* sockfd */(int)regs->regs[MD_REG_A0].q,
		  (void *)&osf_sa, /* addrlen */(int)regs->regs[MD_REG_A2].q);
	...
	for (i=0; i < /* iovcnt */regs->regs[MD_REG_A2].q; i++)
	  {
		...
		if (!buf)
		  fatal("out of virtual memory in SYS_writev");
		mem_bcopy(mem_fn, mem, mc_READ, osf_iov.iov_base,
			  buf, osf_iov.iov_len);
		iov[i].iov_base = buf;
	      }
	    else
	...
	do {
	  /*result*/regs->regs[MD_REG_V0].q =
	    writev(/* fd */(int)regs->regs[MD_REG_A0].q, iov,
		   /* iovcnt */(size_t)regs->regs[MD_REG_A2].q);
	} while (/*result*/regs->regs[MD_REG_V0].q == -1
	         && errno == EAGAIN);
	...
	/* allocate host side I/O vectors */
	osf_iov =
	  calloc(/* iovcnt */regs->regs[MD_REG_A2].q,
		 sizeof(struct osf_iovec));
	if (!osf_iov)
	  fatal("out of virtual memory in SYS_readv");

	iov =
	  calloc(/* iovcnt */regs->regs[MD_REG_A2].q, sizeof(struct iovec));
	if (!iov)
	  fatal("out of virtual memory in SYS_readv");
	...
	do {
	  /*result*/regs->regs[MD_REG_V0].q =
	    readv(/* fd */(int)regs->regs[MD_REG_A0].q, iov,
		  /* iovcnt */(size_t)regs->regs[MD_REG_A2].q);
	} while (/*result*/regs->regs[MD_REG_V0].q == -1
		 && errno == EAGAIN);
	...
	for (i=0; i < /* iovcnt */regs->regs[MD_REG_A2].q; i++)
	  {
	    if (osf_iov[i].iov_base != 0)
	      {
		mem_bcopy(mem_fn, mem, mc_WRITE, osf_iov[i].iov_base,
	...
	if (/* optval */regs->regs[MD_REG_A3].q != 0
	    && /* optlen */regs->regs[MD_REG_A4].q != 0)
	  {
	    buf = calloc(1, /* optlen */(size_t)regs->regs[MD_REG_A4].q);
	    if (!buf)
	      fatal("cannot allocate memory in OSF_SYS_setsockopt");
	    
	    /* copy target side pointer data into host side vector */
	    mem_bcopy(mem_fn, mem, mc_READ,
		      /* optval */regs->regs[MD_REG_A3].q,
		      buf, /* optlen */(int)regs->regs[MD_REG_A4].q);
	  }
	else
	  buf = NULL;

	/* pick the correct translation table */
	if ((int)regs->regs[MD_REG_A1].q == OSF_SOL_SOCKET)
	  {
	    map = sockopt_map;
	    nmap = N_ELT(sockopt_map);
	  }
	else if ((int)regs->regs[MD_REG_A1].q == OSF_SOL_TCP)
	  {
	    map = tcpopt_map;
	...
	else
	  {
	    warn("no translation map available for `setsockopt()': %d",
		 (int)regs->regs[MD_REG_A1].q);
	    map = sockopt_map;
	    nmap = N_ELT(sockopt_map);
	  }

	/* result */regs->regs[MD_REG_V0].q =
	  setsockopt(/* sock */(int)regs->regs[MD_REG_A0].q,
		     /* level */xlate_arg((int)regs->regs[MD_REG_A1].q,
					  socklevel_map, N_ELT(socklevel_map),
					  "setsockopt(level)"),
		     /* optname */xlate_arg((int)regs->regs[MD_REG_A2].q,
					    map, nmap,
					    "setsockopt(opt)"),
		     /* optval */buf,
		     /* optlen */regs->regs[MD_REG_A4].q);
	...
	mem_bcopy(mem_fn, mem, mc_READ,
		  /* paddrlen */regs->regs[MD_REG_A2].q,
	...
	/* result */regs->regs[MD_REG_V0].q =
	  getsockname(/* sock */(int)regs->regs[MD_REG_A0].q,
	...
	/* result */regs->regs[MD_REG_V0].q =
	  getpeername(/* sock */(int)regs->regs[MD_REG_A0].q,
		      /* name */(struct sockaddr *)buf,
		      /* namelen */&addrlen);
    ...
    case OSF_SYS_setgid:
      ...
      /*result*/regs->regs[MD_REG_V0].q =
	setgid(/* gid */(gid_t)regs->regs[MD_REG_A0].q);
    ...
    case OSF_SYS_setuid:
      ...
      /*result*/regs->regs[MD_REG_V0].q =
	setuid(/* uid */(uid_t)regs->regs[MD_REG_A0].q);
    ...
    case OSF_SYS_getpriority:
      ...
      /*result*/regs->regs[MD_REG_V0].q =
	getpriority(/* which */(int)regs->regs[MD_REG_A0].q,
		    /* who */(int)regs->regs[MD_REG_A1].q);
    ...
    case OSF_SYS_setpriority:
      ...
      /*result*/regs->regs[MD_REG_V0].q =
	setpriority(/* which */(int)regs->regs[MD_REG_A0].q,
		    /* who */(int)regs->regs[MD_REG_A1].q,
		    /* prio */(int)regs->regs[MD_REG_A2].q);
	...
	if (/* readfds */regs->regs[MD_REG_A1].q != 0)
	  {
	    mem_bcopy(mem_fn, mem, mc_READ,
		      /* readfds */regs->regs[MD_REG_A1].q,
	...
	if (/* writefds */regs->regs[MD_REG_A2].q != 0)
	  {
	    mem_bcopy(mem_fn, mem, mc_READ,
		      /* writefds */regs->regs[MD_REG_A2].q,
	...
	if (/* exceptfds */regs->regs[MD_REG_A3].q != 0)
	  {
	    mem_bcopy(mem_fn, mem, mc_READ,
		      /* exceptfds */regs->regs[MD_REG_A3].q,
	...
	if (/* timeout */regs->regs[MD_REG_A4].q != 0)
	  {
	    mem_bcopy(mem_fn, mem, mc_READ,
		      /* timeout */regs->regs[MD_REG_A4].q,
	...
	/* result */regs->regs[MD_REG_V0].q =
	  select(/* nfds */regs->regs[MD_REG_A0].q,
	...
	/* copy mc_WRITE file descriptor set to target memory */
	if (/* writefds */regs->regs[MD_REG_A2].q != 0)
	  mem_bcopy(mem_fn, mem, mc_WRITE,
		    /* writefds */regs->regs[MD_REG_A2].q,
		    &writefd, sizeof(fd_set));
    ...
    case OSF_SYS_shutdown:
      ...
      /*result*/regs->regs[MD_REG_V0].q =
	shutdown(/* sock */(int)regs->regs[MD_REG_A0].q,
		 /* how */(int)regs->regs[MD_REG_A1].q);
	...
	/* allocate host side I/O vectors */
	fds = calloc(/* nfds */regs->regs[MD_REG_A1].q, sizeof(struct pollfd));
	if (!fds)
	  fatal("out of virtual memory in SYS_poll");

	/* copy target side I/O vector buffers to host memory */
	for (i=0; i < /* nfds */regs->regs[MD_REG_A1].q; i++)
	  {
	    /* copy target side pointer data into host side vector */
	    mem_bcopy(mem_fn, mem, mc_READ,
		      (/* fds */regs->regs[MD_REG_A0].q
	...
	/* result */regs->regs[MD_REG_V0].q =
	  poll(/* fds */fds,
	       /* nfds */(unsigned long)regs->regs[MD_REG_A1].q,
	       /* timeout */(int)regs->regs[MD_REG_A2].q);
	...
	buf = malloc(/* len */(size_t)regs->regs[MD_REG_A1].q);
	if (!buf)
	  fatal("out of virtual memory in gethostname()");

	/* result */regs->regs[MD_REG_V0].q =
	  gethostname(/* name */buf,
		      /* len */(size_t)regs->regs[MD_REG_A1].q);
	...
	mem_bcopy(mem_fn, mem, mc_WRITE,
		  /* name */regs->regs[MD_REG_A0].q,
		  buf, /* len */regs->regs[MD_REG_A1].q);
      }
      break;
#endif
    ...
    default:
      ...
      warn("invalid/unimplemented system call, code %d, PC=0x%08p, RA=0x%08p, making it abort\n", 
	                                   syscode, regs->PC, regs->regs[MD_REG_RA].q);
      regs->regs[MD_REG_A3].q = -1;
      regs->regs[MD_REG_V0].q = 0;
      break;
    }
  ...
  if (verbose)
    fprintf(stderr, "syscall(%d): returned %d:%d...\n",
            (int)syscode, (int)regs->regs[MD_REG_A3].q,
            (int)regs->regs[MD_REG_V0].q);
}
