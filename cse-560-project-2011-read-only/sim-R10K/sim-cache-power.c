...
itlb_miss_handler(enum mem_cmd_t cmd,	/* access cmd, Read or Write */
		 ...
		 unsigned int bsize,	/* size of block to access */
		 tick_t now,
		 bool_t miss_info[ct_NUM])	/* time of access */
{
  if (cache_l2)
    {
      power_count_access(ps_L2_TAG, /* write_f */FALSE, /* hard_count_f */FALSE);
		 ...
		 bool_t miss_info[ct_NUM])	/* time of access */
{
  ...
  power_count_access(ps_DTLB, /* write_f */TRUE, /* hard_count_f */TRUE);
  return /* access latency, ignored */0;
}
...
{
  if (cache_il1)
    cache_access(cache_il1, mc_READ, regs.PC, sizeof(md_inst_t), 0, NULL, il1_miss_handler);
  if (itlb)
    cache_access(itlb, mc_READ, regs.PC, sizeof(md_inst_t), 0, NULL, itlb_miss_handler);
  ...
  if (pdi->iclass == ic_load || pdi->iclass == ic_store || pdi->iclass == ic_prefetch)
    {
      ...
      if (cache_dl1)
	cache_access(cache_dl1, dl1_cmd, regs.addr, regs.dsize, 0, miss_info, dl1_miss_handler);
      if (dtlb)
	cache_access(dtlb, dtlb_cmd, regs.addr, regs.dsize, 0, NULL, dtlb_miss_handler);
    }
}
...
power_init(void)
{
  ...
  if (cache_il1)
  {
    ...
    cache_onebank_power(/* nsets */cache_il1_opt.nsets,
			...
			/* osize */sizeof(md_inst_t) * 8,
			/* bsize */sizeof(md_inst_t) * 8,
  ...
  if (cache_dl1)
  {
    ...
    cache_onebank_power(/* nsets */cache_dl1_opt.nsets,
			...
			/* data_write_power */&w_power);
    max_power = r_power;
    power_set_power_nports(ps_DL1_DATA, r_power, w_power, max_power, 0, 0, 1);
    max_power = t_r_power;
    power_set_power_nports(ps_DL1_TAG, t_r_power, t_w_power, max_power, 0, 0, 1);
  }
  ...
  if (dtlb)
  {
    ...
    cam_array_power(dtlb_opt.nsets, MD_VADDR_SIZE - log_base2(dtlb_opt.bsize), /* rport */1, /* wport */1,
		    &t_r_power, &t_w_power);
    simple_array_power(dtlb_opt.nsets, MD_PADDR_SIZE - log_base2(dtlb_opt.bsize),
		       /* rport */1, /* wport */1, /* cache? */FALSE,
		       &r_power, &w_power);
    max_power = (r_power + t_r_power) + (w_power + t_w_power); 
    power_set_power_nports(ps_DTLB, r_power + t_r_power, w_power + t_w_power, max_power, 1, 1, 0);
  }
...
sim_aux_stats(FILE *stream)		/* output stream */
{
  ...
  power_stats_print(sim_sample_insn, stream);
}
...
sim_sample(unsigned int n_insn)
{
  ...
  while (n_insn == 0 || sim_num_insn < sim_num_insn_begin + n_insn)
	...
	{
	  cache_access(cache_il1, mc_READ, regs.PC, sizeof(md_inst_t), 0, NULL, il1_miss_handler);
	  power_count_access(ps_IL1_TAG, /* write_f */FALSE, /* hard_count_f */TRUE);
	  power_count_access(ps_IL1_DATA, /* write_f */FALSE, /* hard_count_f */TRUE);
	}
	  ...
	  if (cache_dl1)
	    {
	      ...
	      cache_access(cache_dl1, dl1_cmd, regs.addr, regs.dsize, 0, miss_info, dl1_miss_handler);
	      power_count_access(ps_DL1_TAG, /* write_f */FALSE, /* hard_count_f */TRUE);

	      if (pdi->iclass == ic_store)
		power_count_access(ps_DL1_DATA, /* write_f */TRUE, /* hard_count_f */TRUE);
	      else if (pdi->iclass == ic_load)
		power_count_access(ps_DL1_DATA, /* write_f */FALSE, /* hard_count_f */TRUE);
	      /* prefetches do not access the data array */
	    }
...
sim_sample_on(unsigned long long n_insn)
{
  ...
  if (n_insn == 0)
    fprintf(stderr, "** sim -- simulating caches until program completion **\n");
  else
    fprintf(stderr, "** sim -- simulating caches %llu instructions **\n", n_insn);
